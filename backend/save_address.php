...
if ($exists) {
    // update
    $stmt = $pdo->prepare("UPDATE addresses SET name=?, phone=?, address_line1=?, address_line2=?, city=?, state=?, pincode=? WHERE id=?");
    $stmt->execute([$name,$phone,$line1,$line2,$city,$state,$pincode,$exists['id']]);
    $addr_id = $exists['id'];
} else {
    // insert
    $stmt = $pdo->prepare("INSERT INTO addresses(user_id,name,phone,address_line1,address_line2,city,state,pincode) VALUES(?,?,?,?,?,?,?,?)");
